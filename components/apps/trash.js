...
const App = (props) => {
  // Logic to determine initial folder (Home vs Trash)
  ...
  return (
    <div className={`flex flex-col h-full w-full bg-[#0f1115] text-[#e1e3e6] font-sans select-none overflow-hidden transition-all duration-500 ${!isMaximized ? 'rounded-2xl border border-[#30343d] shadow-2xl scale-[0.98]' : ''}`}>
      ...
      {/* Trash Header Overlay (appears in /trash folder) */}
      {currentPath === '/trash' && (
          <div className="flex items-center justify-between px-6 py-2 bg-[#2a2e35]/40 border-b border-white/5 animate-in fade-in duration-300">
              <span className="text-[11px] font-bold text-gray-400 flex items-center gap-2 uppercase tracking-widest">
                  <Trash2 size={12} /> System Trash Repository
              </span>
              <div className="flex gap-2">
                  <button onClick={restoreTrash} className="flex items-center gap-2 px-3 py-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded-md text-[11px] font-bold transition-all text-gray-300">
              ...
              <div className="flex-1 overflow-auto p-4 md:p-6 bg-[#0a0c0f] rounded-b-2xl relative">
                ...
                {openFile.type === 'image' && (
                  <div className="flex items-center justify-center h-full relative group">
                    <button onClick={() => navigateImage('prev')} className="absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80 z-20"><ChevronLeft size={32} /></button>
                    ...
                    <button onClick={() => navigateImage('next')} className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80 z-20"><ChevronRight size={32} /></button>
                  </div>
                )}
      ...
      <div className="bg-[#181a1f] border-t border-[#2a2e35] px-5 py-2 flex justify-between items-center text-[10px] md:text-[11px] text-gray-500 font-bold tracking-tight">
        <div className="flex gap-4 md:gap-8 items-center">
          ...
          <span className="hidden sm:flex items-center gap-2 hover:text-gray-300 transition-colors cursor-default uppercase"><HardDrive size={12} /> 332.6 GB available</span>
        </div>
...
/**
 * EXPORTS: Update your apps.config.js to use these correctly.
 */
