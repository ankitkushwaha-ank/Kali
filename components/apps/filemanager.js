    ...
    <div className={`flex flex-col h-full w-full bg-[#0f1115] text-[#e1e3e6] font-sans select-none overflow-hidden transition-all duration-500 ${!isMaximized ? 'rounded-2xl border border-[#30343d] shadow-2xl scale-[0.98]' : ''}`}>
      ...
      <div className="flex items-center gap-1 px-2 py-1 bg-[#181a1f] border-b border-[#2a2e35] text-[12px] relative z-[60]" ref={menuRef}>
        ...
        <div className="flex items-center gap-1.5 pr-2">
            ...
            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Secure Node</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 bg-[#111318] border-b border-[#2a2e35] shadow-inner">
        <div className="flex items-center gap-1">
          <button 
            ...
            className={`w-9 h-9 flex items-center justify-center rounded-xl transition-all ${historyIndex === 0 ? 'opacity-20 cursor-not-allowed text-gray-600' : 'hover:bg-white/5 text-blue-400'}`}
          >
        ...
        <div className="hidden lg:flex items-center gap-2">
          <div className="relative group">
            ...
            <input 
              type="text" 
              placeholder="Search..." 
              className="bg-[#0a0c0f] border border-[#2a2e35] rounded-full pl-10 pr-4 py-2 text-[12px] w-40 focus:w-56 transition-all outline-none text-gray-200"
            />
          </div>
          <button className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-white/5 text-gray-400">
            <LayoutGrid size={20} />
          </button>
        </div>
      </div>
      ...
      {currentPath === '/trash' && (
          <div className="flex items-center justify-between px-6 py-2 bg-[#2a2e35]/30 border-b border-white/5 animate-in fade-in duration-300">
              <span className="text-[11px] font-bold text-gray-400 flex items-center gap-2">
                  <Trash2 size={12} /> TRASH REPOSITORY
              </span>
              <div className="flex gap-2">
                  <button onClick={restoreTrash} className="flex items-center gap-2 px-3 py-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded-md text-[11px] font-bold transition-all">
                      <RefreshCcw size={12} /> Restore
                  </button>
                  <button onClick={emptyTrash} className="flex items-center gap-2 px-3 py-1 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 rounded-md text-[11px] font-bold transition-all">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {/* Content View */}
        <div 
          className="flex-1 p-4 md:p-6 bg-[#0f1115] overflow-y-auto custom-scrollbar relative"
              ...
              return (
                <div 
                  ...
                  className={`flex flex-col items-center p-3 rounded-xl cursor-default transition-all group border ${
                    isSelected 
                      ? 'bg-[#3b82f6]/20 text-white border-[#3b82f6]/40 shadow-xl scale-105 z-10' 
                      : 'hover:bg-white/5 border-transparent'
                  }`}
                >
                  <div className="mb-2 relative transition-transform active:scale-95">
                    {icon}
                  </div>
                  <span className={`text-[11px] md:text-[12px] text-center break-all line-clamp-2 px-1.5 rounded leading-tight font-medium ${
                    isSelected ? 'text-white bg-[#3b82f6] rounded px-2 shadow-lg' : 'text-gray-300'
                  }`}>
                    {itemName}
                  </span>
                </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-gray-700 opacity-40 animate-in fade-in zoom-in-95">
              {currentPath === '/trash' ? (
                  <div className="flex flex-col items-center">
                    ...
                    <button onClick={restoreTrash} className="mt-4 text-[10px] underline hover:text-white transition-colors cursor-pointer">Restore System Trash Data</button>
                  </div>
              ) : (
                  <div className="flex flex-col items-center">
            ...
            <div className="absolute inset-2 md:inset-6 bg-[#181a1f] border border-[#2a2e35] shadow-[0_30px_60px_rgba(0,0,0,0.8)] rounded-2xl z-[100] flex flex-col animate-in">
              <div className="flex items-center justify-between bg-[#111318] border-b border-[#2a2e35] px-4 py-3 rounded-t-2xl">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setOpenFile(null)} className="p-1.5 hover:bg-[#ef4444] text-gray-500 hover:text-white rounded-lg transition-all">
                      <X size={18} />
                    </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-auto p-4 md:p-6 bg-[#0a0c0f] rounded-b-2xl relative">
                {openFile.type === 'text' && (
                  <textarea 
                    className="w-full h-full bg-transparent text-gray-300 font-mono text-[13px] leading-relaxed outline-none resize-none custom-scrollbar"
                ...
                {openFile.type === 'image' && (
                  <div className="flex items-center justify-center h-full relative group">
                    <button onClick={() => navigateImage('prev')} className="absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"><ChevronLeft size={32} /></button>
                    ...
                    <button onClick={() => navigateImage('next')} className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"><ChevronRight size={32} /></button>
                  </div>
                )}
                {openFile.type === 'pdf' && (
                  <iframe 
                    src={openFile.url} 
                    className="w-full h-full rounded-xl border border-white/5 bg-white opacity-95"
                    title="PDF Viewer"
                  />
                )}
              </div>
      ...
      <div className="bg-[#181a1f] border-t border-[#2a2e35] px-5 py-2 flex justify-between items-center text-[10px] md:text-[11px] text-gray-500 font-bold tracking-tight">
        <div className="flex gap-4 md:gap-8 items-center">
          <span className="flex items-center gap-2 hover:text-gray-300 transition-colors cursor-default">
             <File size={12} /> {items.length} objects
          </span>
          <span className="hidden sm:flex items-center gap-2 hover:text-gray-300 transition-colors cursor-default uppercase">
             <HardDrive size={12} /> 332.6 GB available
          </span>
        </div>
