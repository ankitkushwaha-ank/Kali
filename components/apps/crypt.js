  ...
  const SidebarLink = ({ icon: Icon, label, path, viewKey }) => {
    ...
    return (
      <div 
        ...
        className={`flex items-center gap-3 px-3 py-2.5 cursor-default transition-all rounded-lg mx-2 ${
          isActive 
  ...
  return (
    <div className={`flex flex-col h-full w-full bg-[#0f1115] text-[#e1e3e6] font-sans select-none overflow-hidden transition-all duration-500 ${!isMaximized ? 'rounded-2xl border border-[#30343d] shadow-2xl scale-[0.98]' : ''}`}>
      ...
      <div className="flex items-center gap-1 px-2 py-1 bg-[#181a1f] border-b border-[#2a2e35] text-[12px] relative z-[60]" ref={menuRef}>
        <button onClick={() => setShowMobileSidebar(!showMobileSidebar)} className="md:hidden p-2 hover:bg-white/5 rounded-lg text-blue-400 transition-colors">
           <Menu size={20} />
        </button>
        {Object.keys(MENU_DATA).map((menu) => (
          <div key={menu} className="hidden md:block relative">
            ...
            {activeMenu === menu && (
              <div className="absolute top-[calc(100%+2px)] left-0 min-w-[180px] bg-[#181a1f] border border-[#2a2e35] shadow-2xl rounded-lg z-[100] py-1.5 backdrop-blur-md animate-in fade-in zoom-in-95">
        ...
        <div className="flex items-center gap-1.5 pr-2">
            ...
            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest hidden sm:inline">Production ESR</span>
        </div>
      </div>

      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-2 p-2 md:p-3 bg-[#111318] border-b border-[#2a2e35] shadow-inner">
        ...
        <div className="flex-1 min-w-[150px] flex items-center bg-[#0a0c0f] border border-[#2a2e35] rounded-xl h-10 px-4 shadow-inner focus-within:ring-2 focus-within:ring-[#3b82f6]/30 transition-all">
          <Folder size={14} className="text-blue-500 mr-2 flex-shrink-0" />
          <input 
            type="text" 
            value={addressBarValue}
            onChange={(e) => setAddressBarValue(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && navigateTo(addressBarValue)}
            className="w-full text-[13px] outline-none bg-transparent text-gray-300 font-mono tracking-tight truncate"
          />
        </div>
      ...
      {/* Trash Header Overlay */}
      {currentPath === '/trash' && (
          <div className="flex items-center justify-between px-4 md:px-6 py-2 bg-[#2a2e35]/40 border-b border-white/5 animate-in fade-in">
              <span className="text-[10px] md:text-[11px] font-bold text-gray-400 flex items-center gap-2 uppercase tracking-widest truncate">
                  <Trash2 size={12} /> System Trash
              </span>
              <div className="flex gap-2">
                  <button onClick={restoreTrash} className="flex items-center gap-2 px-2 md:px-3 py-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded-md text-[10px] md:text-[11px] font-bold transition-all text-gray-300">
                      <RefreshCcw size={12} /> <span className="hidden sm:inline">Restore</span>
                  </button>
                  <button onClick={emptyTrash} className="flex items-center gap-2 px-2 md:px-3 py-1 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 rounded-md text-[10px] md:text-[11px] font-bold transition-all">
                      <Trash2 size={12} /> <span className="hidden sm:inline">Empty</span>
                  </button>
              </div>
          </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`
          ...
          md:translate-x-0 absolute md:relative z-50 md:z-auto w-64 md:w-60 h-full bg-[#14171c] border-r border-[#2a2e35] py-4 flex flex-col gap-6 overflow-y-auto transition-transform duration-300 ease-in-out shadow-2xl md:shadow-none
        `}>
          <div className="flex items-center justify-between px-6 md:hidden">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-[2px]">Kali Menu</span>
              <button onClick={() => setShowMobileSidebar(false)} className="p-1 hover:bg-white/5 rounded-md text-gray-500"><X size={18}/></button>
          </div>
          <div>
        ...
        {/* Content View */}
        <div className="flex-1 p-3 md:p-6 bg-[#0f1115] overflow-y-auto custom-scrollbar relative" onClick={() => setSelectedItem(null)}>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(90px,1fr))] md:grid-cols-[repeat(auto-fill,minmax(110px,1fr))] gap-2 md:gap-3">
            {items.map((itemName) => {
              const fullPath = currentPath === '/' ? `/${itemName}` : `${currentPath}/${itemName}`;
              ...
              const icon = isFolder 
                ? <Folder size={44} className="md:size-12 text-blue-500 fill-blue-500/10 filter drop-shadow-md" /> 
                : (fileInfo?.icon || <File size={44} className="md:size-12 text-gray-500" />);
              ...
              return (
                <div 
                  ...
                  className={`flex flex-col items-center p-2.5 md:p-3 rounded-xl cursor-default transition-all group border ${
          ...
          {items.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-gray-700 opacity-40 animate-in fade-in">
              {currentPath === '/trash' ? (
                  <div className="flex flex-col items-center text-center">
                    <Trash2 size={80} strokeWidth={1} className="mb-4" />
                    <p className="text-lg font-bold tracking-widest uppercase text-gray-500">Trash is empty</p>
                    <button onClick={restoreTrash} className="mt-4 text-[10px] underline hover:text-white transition-colors cursor-pointer">Restore Mock Items</button>
                  </div>
              ) : (
                  <div className="flex flex-col items-center">
                    <Folder size={80} strokeWidth={1} className="mb-4" />
                    <p className="text-lg font-bold tracking-widest uppercase text-gray-500">Empty Directory</p>
                  </div>
              )}
            </div>
          )}
          ...
          {/* VIEWER OVERLAY */}
          {openFile && (
            <div className="absolute inset-0 md:inset-6 bg-[#181a1f] border border-[#2a2e35] shadow-[0_30px_60px_rgba(0,0,0,0.8)] md:rounded-2xl z-[100] flex flex-col animate-in">
              <div className="flex items-center justify-between bg-[#111318] border-b border-[#2a2e35] px-4 py-3 md:rounded-t-2xl">
                <div className="flex items-center gap-3">
                  <button onClick={() => setOpenFile(null)} className="md:hidden p-1 mr-1 text-blue-400"><ArrowLeft size={20}/></button>
                  <ShieldCheck size={16} className="text-blue-500 hidden sm:block" />
                  <span className="text-[11px] font-black text-gray-500 uppercase tracking-widest truncate max-w-[200px] md:max-w-none">{openFile.name}</span>
                </div>
                <div className="flex items-center gap-2 md:gap-4">
                    {openFile.type === 'image' && getImagesInCurrentDir().length > 1 && (
                        <div className="flex items-center gap-2 bg-black/40 rounded-lg p-1">
                            <button onClick={() => navigateImage('prev')} className="p-1 hover:bg-white/10 rounded-md transition-colors"><ChevronLeft size={16} /></button>
                            <span className="text-[10px] font-mono text-gray-500 px-1">{getImagesInCurrentDir().indexOf(openFile.name) + 1} / {getImagesInCurrentDir().length}</span>
                            <button onClick={() => navigateImage('next')} className="p-1 hover:bg-white/10 rounded-md transition-colors"><ChevronRight size={16} /></button>
                        </div>
                    )}
                    <button onClick={() => setOpenFile(null)} className="p-1.5 hover:bg-[#ef4444] text-gray-500 hover:text-white rounded-lg transition-all hidden md:flex"><X size={18} /></button>
                </div>
              </div>
              <div className="flex-1 overflow-auto p-4 md:p-6 bg-[#0a0c0f] md:rounded-b-2xl relative">
                ...
                {openFile.type === 'image' && (
                  <div className="flex items-center justify-center h-full relative group">
                    <button onClick={() => navigateImage('prev')} className="absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80 z-20 hidden md:flex"><ChevronLeft size={32} /></button>
                    <img key={openFile.url} src={openFile.url} alt={openFile.name} className="max-w-full max-h-full rounded-lg shadow-2xl transition-opacity duration-300" />
                    <button onClick={() => navigateImage('next')} className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80 z-20 hidden md:flex"><ChevronRight size={32} /></button>
                  </div>
                )}
      ...
      <div className="bg-[#181a1f] border-t border-[#2a2e35] px-5 py-2 flex justify-between items-center text-[10px] md:text-[11px] text-gray-500 font-bold tracking-tight">
        <div className="flex gap-4 md:gap-8 items-center">
          <span className="flex items-center gap-2 hover:text-gray-300 transition-colors cursor-default"><File size={12} /> {items.length} <span className="hidden sm:inline">objects</span></span>
          <span className="hidden sm:flex items-center gap-2 hover:text-gray-300 transition-colors cursor-default uppercase"><HardDrive size={12} /> 332.6 GB free</span>
        </div>
        <div className="flex items-center gap-3 md:gap-5 overflow-hidden">
          <span className="font-mono text-[#3b82f6]/80 truncate max-w-[150px] sm:max-w-none">{currentPath}</span>
...
/**
 * EXPORTS: Update your apps.config.js to use these.
 */
export const displayfilemanager = () => <App />;
export const displayTrash = () => <App initialPath="/trash" />;
export const displayCrypt = () => <App initialPath="/home/kali/Desktop/Crypt" />;